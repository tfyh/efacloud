...
if ($done > 0) {
    ...
    if (strlen($form_errors) > 0) {
        ...
        if (isset($entered_data['EMail']) && (strlen($entered_data['EMail']) > 0)) {
            ...
            if (($mail_address_used !== false) && (intval(
                    $mail_address_used[$toolbox->users->user_id_field_name]) !==
                     intval($toolbox->users->session_user["@id"]))) {
                $form_errors .= i('SC6SXl|The email address °%1° i...', $entered_data["EMail"], 
                        $mail_address_used["Vorname"], $mail_address_used["Nachname"]) . " ";
            }
        }
    ...
    if ($todo == 2) {
        ...
        foreach ($user_to_update as $key => $value) {
            ...
            $user_has_password = isset($toolbox->users->session_user["Passwort"]);
            ...
            if (isset($entered_data[$key]) && $log_modification) {
                $info .= i("GsvRx8|%1 was changed from °%2°...", $key, $user_to_update[$key], 
                        $entered_data[$key]) . "<br>";
        ...
        if (! $form_errors) {
            ...
            $form_errors = $socket->update_record($toolbox->users->session_user["@id"], 
            ...
            $info_html = '<p><b>Daten geändert.</b><br/>".i("Folgende Änderungen wurden vorgenommen:")."<br>' .
                     $info . '</p><p><a href = "../pages/mein_profil.php">".i("Zurück zum Profil.")."</a></p>';
            ...
            $mail_handler->send_mail($mail_handler->system_mail_sender, $mail_handler->system_mail_sender, 
                    ...
                    i("e1szHW|Change of profile data a...", $toolbox->users->session_user["@id"]), 
...
if (isset($form_filled) && ($todo == $form_filled->get_index())) {
    ...
    if (($todo == 1)) {
        ...
        $user_to_update = $socket->find_record_matched($toolbox->users->user_table_name, 
                ["ID" => $toolbox->users->session_user["ID"]
                ], false);
