...
var cForm = {
    ...
    init : function(formName) {
        ...
        formConfig.forEach(function(fieldDefinition) {
			...
			// definition options.
        	// replace texts for i18n
            if (cToolbox.isI18nResourceId(fieldDefinition["value"]))
        		fieldDefinition["value"] = _(fieldDefinition["value"]);
            if (cToolbox.isI18nResourceId(fieldDefinition["label"]))
			...
			// They will become "_help_text1", "_help_text2" etc. Same with
			// "_no_input".
            if (help_text) {
                iht ++;
    ...
    _readOptions:function(fieldDefinition) {
        if (fieldDefinition["type"].trim().toLowerCase().indexOf("select use:") == 0) {
            ...
            parameter_name = (lookup.length == 2) ? lookup[1] : _("xbtBBx|select use syntax error ...");
            select_string = "";
            if ($_params) {
            ...
            if (select_string.length == 0)
                fieldDefinition["type"] = _("iyQl8o|parameter_not_initialize...") + "=" + _("yTgnip|parameter_not_initialize...");
    ...
    getSelectOptionsArray : function(f) {
		...
		// special case: the selection is provided programmatically based on an
		// object
        ...
        for (let key in this.inputs) {
            ...
            if (value.length < 1) {
                ...
                if (definition["type"].length > 0) {
                    ...
                    if (type.localeCompare("email") == 0) {
                        ...
                        if (cToolbox.checkAndFormatDate(value) == false) {
                            formErrors += _("rYOCmN|Please enter a valid dat...",
                            		definition["label"], value) + '<br>';
                            this.validities[key] = false;
                        }
                    } else if ((type.localeCompare("password") == 0) && (password_rule > 0)) {
                        errors = cToolbox.checkPassword(value);
                        if (errors.length > 0) {
                            formErrors += _("johtdS|The password is not secu...",
                ...
                if ((definition["class"].length > 9) && (definition["class"].indexOf("validate:") == 0)) {
                    ...
                    if (! guid) {
                        formErrors += _("yJhC53|The name °%1° was not fo...",
                        		definition["label"], definition["class"].substring(9)) + '<br>';
                        this.validities[key] = false;
                    }
