...
var efaListDefs = {
	...
	/**
	 * list of efaWeb lists which a numeric ID index is created.
	 */
	indexNumericIdsFor : {
	...
	/**
	 * list of efaWeb lists which a numeric ID-list index is created.
	 */
	indexNumericIdListsFor : {
		efaWeb_logbook : "EntryId",
	},
	 ...
	 * list of tables which hav an IdList field
	 */
	usedForSpecialIndices : [ "efaWeb_logbook", "efaWeb_own_sessions", "efaWeb_persons" ],
	...
	buildSpecialIndices : function(listname) {
		...
		if ((listname.localeCompare("efaWeb_logbook") == 0) || (listname.localeCompare("efaWeb_own_sessions") == 0)) {
			// standard index for entry IDs is built before this.
			// index for last trips. Prepare.
			var lastSessions = [];
			var r = 0;
			if (!cLists.lists.efaWeb_logbook)
	...
	getOptions : function(listname) {
		...
		if (listname.toLowerCase().localeCompare("lastSessions")) {
			var lastSessions = [];
			lastSessions.push("0=" + _("jMcbIg|please select"));
