...
var sFormHandler = {
	...
	add_new_prepare : function() {
        if (formIsNewRecord) {
        	...
        	$('#editaction').html("neu anlegen");
			$("input[type=submit]").val("Jetzt neu anlegen");
        }
	},
	...
	versionized_prepare : function() {
        ...
        if (formIsNewRecord) {
        	$('#valDateLabel').html("Gültig ab Datum");
        	$('#valTimeLabel').html("Gültig ab Zeit");
			...
			$('#versionized-data').show();
			$("input[type=submit]").val("Daten für '" + name + "' ändern");
		} else { 
			$('#vDiv-ValidityFromDate').show();
			$('#vDiv-ValidityFromTime').show();
			$('#versionized-data').hide();
			if (vWayOfChange === 1) 
				$("input[type=submit]").val("Gültig für '" + name + "' ändern");
			else {
	        	$('#valDateLabel').html("Abgrenzen zu Datum");
	        	$('#valTimeLabel').html("Abgrenzen zu Zeit");
				$("input[type=submit]").val("Eine neue Version für '" + name + "' anlegen");
			}
		} 
	},
	...
	add_group_member : function() {
		...
		if (memberNameList.includes(memberName))
			alert("Die Person " + memberName + " ist bereits in der Gruppe enthalten.");
		else {
	...
	is_change_forbidden : function() {
		if ($("input[name=EntryId]").length > 0) {
			// logbook form
			var openId = $("input[name=Open]").attr("id");
			if (openId.localeCompare("checked-off") != 0) {
				$("input[type=submit]").val("Die Fahrt ist offen und darf daher nicht geändert werden.");
	...
	editRecord_prepare : function() {
		var inputs = $(":input");
		// start with special case: select the correct boat variant for a sessien record
		var boatVariant = 1;
				...
				.each(function(i, element) {
					...
					if (sFormHandler.resolve[inputName]) {
						...
						sFormHandler.checkBoxStates[inputName] = ($(element)
								.attr("id").localeCompare("checked-on") == 0);
					}
				});
		// register all changes for form submit
		// this will run in parallel to the autocompletion. Checks there will validate the value.
		// Note that 
		inputs
				.on(
						"keyup change",
		...
		if (sFormHandler.resolve[inputName]) {
			...
			if (isList) {
				...
				changedNames.forEach(function(cn) {
					// if one Id in the list was matched, drop the unmatched names
	...
	showAllChanges : function() {
		...
		if (changeText !== '')
			$('#changed-values-text').html(
					"<br><b>geändert wird:</b><br>" + changeText);
		else
