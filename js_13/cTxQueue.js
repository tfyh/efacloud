...
var cTxQueue = {
	...
	buildContainer : function(txs) {
		...
		var base64api = base64.replace(/\//g, '-').replace(/\+/g, '*').replace(
				/\=/g, '_');
		console.log("V" + this.apiVersion + " container built: " + txs.length + " transactions in " + base64api.length + " base64 characters");
		return base64api;
	},
	...
	addNewTxToPending : function(type, tablename, record, retries, callback, onError) {
		...
		console.log("Pending queue ++: " + cTxQueue.queues.txPending.length);
		return tx;
	},
	...
	parseResponse : function(response) {
		...
		if (txc.cApiVersion > this.apiMaxVersion) {
			...
			txc.cresultMessage = "WARNING: API version of container: " + txc.cApiVersion 
								+ " not supported. Maximum API level: " + this.apiMaxVersion;
		}
		// syntax error check.
	...
	handleResponse : function(txrc) {
		...
		for (txr of txrc.txrs) {
			...
			if (tx) {
				...
				console.log("WARNING: transaction #" + tc + " of " + txrc.txrs.length + " is undefined @cTxQueue.handleResponse.");
			}
			tc ++;
		}
	},
