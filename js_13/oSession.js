...
var oSession = {
	...
	buttonEndSession : '<div class="w3-col l2">'
			+ '<p><span class="formbutton" id="do-getForm_endSession_[EntryId]"><b>Fahrt beenden</b></span>'
			+ '&nbsp;</p></div>',
	buttonUpdateSession : '<div class="w3-col l4">'
			+ '<p style="text-align:right"><span class="formbutton" id="do-getForm_updateSession_[EntryId]">Fahrt ändern</span>'
			+ '&nbsp;</p></div>',
	buttonCancelSession : '<div class="w3-col l4">'
			+ '<p style="text-align:right"><span class="formbutton" id="do-getForm_cancelSession_[EntryId]">Fahrt abbrechen</span>'
			+ '&nbsp;</p></div>',
	buttonRowEnd : '</div>',
	...
	logbookText : function(session) {
		...
		return session.EntryId + ": " + cToolbox.dateISO2DE(session.Date) + " "
				...
				+ oSession.getBoatName(session) + ", "
				+ oSession.getDestinationName(session) + " ("
				+ session.Distance + "). Am Steuer: '"
				+ this.getCrewName(session, "Cox") + "', im Boot '" + crew
				+ "'";
	},
	...
	getDatasheet : function(session) {
		...
		var endButton = (isOpen) ? oSession.buttonEndSession.replace(
				"[EntryId]", "" + session.EntryId) : "";
		var updateButton = (isOpen) ? oSession.buttonUpdateSession.replace(
				"[EntryId]", "" + session.EntryId) : "";
		var cancelButton = (isOpen) ? oSession.buttonCancelSession.replace(
				"[EntryId]", "" + session.EntryId) : "";
	...
	updateModal : function(sessions) {
		...
		var endButton = (sessions[0].Open.localeCompare("true") == 0) ? oSession.buttonEndSession
				.replace("[EntryId]", "" + sessions[0].EntryId)
				: "";
