 ...
 * "tags;required;name;value;label;type;class;size;maxlength" and the following lines the respective
 * values.</p> <p>The form is always displayed as a table grid. Use the "tags" definition section to provide
 * the table tags, which define the grid.</p> <p> Here is an example:
 * <br>&nbsp;&nbsp;tags;required;name;value;label;type;class;size;maxlength<br>
 * &nbsp;&nbsp;&lt;tr&gt;&lt;td&gt;;*;first;John;please enter first name;text;;20;250<br>
 * &nbsp;&nbsp;&lt;/td&gt;&lt;td&gt;;*;name;Doe;please enter last name;text;;20;250<br>
 * &nbsp;&nbsp;&lt;/td&gt;&lt;/tr&gt;&lt;tr&gt;&lt;td&gt;;;birthday;;please enter date of birth
 * (optional);date;;;<br>&nbsp;&nbsp;"&lt;/td&gt;&lt;/tr&gt;&lt;tr&gt;&lt;td
 * colspan=""3""&gt;";;submit;Accept;;submit;fvssp-button;; </p> <p>Following special options exist for a form
 * element:</p> <ul> <li>Input type select: set type to "select value1=display1;value2=display2 ...".</li>
 * <li>Input type radio: set type to "radio value1=display1;value2=display2 ...".</li> <li>Input type
 * textarea: set size=count of rows, maxlength=width in characters.</li> <li>Mandatory entry: set "required"
 * to "*" to trigger the validity check to expect an entry.</li> <li>Name _help_text: if the name equals
 * '_help_text' it will not be returned in get_html(), but rather in get_help_html(). This name can be used
 * more than once.</li> <li>Name _no_input: if the name equals '_no_input' only the label is used. This name
 * can be used more than once. Used to add explanations in forms.</li> </ul> <p>The form will always use the
 * action "?fseq=[randomNo][$form_index]", so that re-enters the same php-page after form completion with
 * the done value parameter set. Check the [$form_index] value to know what form to display next. The random
 * number is used to link the entered data to the form sequence which is needed in case a user has opened two
 * forms of the same type in the same session, e. g. in two different tabs. In multistep forms, steps 2
 * through x forms must use the layout name [form]_[step], e.g. lyaout #1 is 'registration', then #2 is
 * 'registration_2'.</p><p>The Form class provides security functions: it reads data provided in the http
 * request as GET or POST field and fills an array with those. Reading includes replacement of "`" by the
 * Armenian apostrophe and ";" by the Greek question mark. Characters look similar, but have different code
 * points so that they will not be interpreted in their SQL-function such by any data base. To prevent from
 * cross side scripting, "<" is replaced by the math preceding character.</p><p>To display a Form, create a
 * PHP File using a "done" and "todo" counter.</p><p>The "done"-value provides the information of the executed
 * workflow step and rules the application logic. The entered values shall be read and validated in the first
 * PHP-code part. Construct a Form using the done worklow step and run the "read_entered" function to read the
 * values. Compile all generic error via the "check_validity" function. Apply workflow specific application
 * logic afterwards. Collect all errors.</p><p>Now, in a second code part, the form and texts are displayed
 * using the "get_html" function. If there were errors, can repeat the step by reusing the same Form instance.
 * "get_html" will provide you with that form showing all entered values and red-flagged erroneous fields. If
 * the validation succeeded (or upon workflow start), construct a new, empty Form for data entry. You should
 * rule the page display code part by the "todo"-parameter.</p>
 */
class Form
{
    ...
    /**
     * The name of the table, to which this forms data are put.
     */
    private $table_name;
     ...
     *            the form is always associated to a table of the data base. Provide the table name
     *            here.
     * @param int $index
     ...
     *            multisequence form. Set to "" to generate a new one. Will be passed to the posted
     *            form as "fseq" get-parameter prefixing the done parameter.
     * @param array $allowed_roles
     *            An array of roles allowed for the user who uses the form to steer the options
     *            selection.
     */
    public function __construct (String $file_path, Socket $socket, Toolbox $toolbox, String $table_name, int $index, 
            String $fs_id = "")
    {
        $this->socket = $socket;
        $this->toolbox = $toolbox;
        $this->table_name = $table_name;
        $this->fs_id = (strlen($fs_id) == 5) ? $fs_id : $toolbox->generate_token(5, true);
        if ($index === 1) {
        ...
        foreach ($form_definition as $field_definition) {
            ...
            if ($help_text) {
                ...
                // expand the subscriptions definition in a field per subscription.
                // subscription names are already unique and must be used unchanged
                $subscriptions_set = $toolbox->read_csv_array("../config/access/subscriptions");
                foreach ($subscriptions_set as $subscription) {
                    $field_definition_subs = $field_definition;
                    // replace in all field definition values all subscription keys by their
                    // subscription values
                    foreach ($field_definition as $key => $value)
                        if (strpos($value, "#") !== false)
                            foreach ($subscription as $skey => $svalue)
                                $field_definition_subs[$key] = str_replace("#" . $skey, $svalue, 
                                        $field_definition_subs[$key]);
                    $this->form_definition[$field_definition_subs["name"]] = $field_definition_subs;
                }
            } elseif ($workflows) {
                // expand the workflows defintion in a field per workflow.
                // workflow names are already unique and must be used unchanged
                $workflows_set = $toolbox->read_csv_array("../config/access/workflows");
                foreach ($workflows_set as $workflow) {
                    $field_definition_works = $field_definition;
                    // replace in all field definition values all workflow keys by their workflow
                    // values
                    foreach ($field_definition as $key => $value)
                        if (strpos($value, "@") !== false)
                            foreach ($workflow as $wkey => $wvalue)
                                $field_definition_works[$key] = str_replace("@" . $wkey, $wvalue, 
                                        $field_definition_works[$key]);
                    $this->form_definition[$field_definition_works["name"]] = $field_definition_works;
                }
            } else {
        ...
        $this->index = $index;
        file_put_contents("../log/formdebug.log", 
                date("Y-m-d H:i.s") . " - Form " . $file_path . " (" . $this->fs_id . "|" . $index . "): " .
                         json_encode($_SESSION["forms"][$this->fs_id]) . "\n", FILE_APPEND);
    }
    ...
    private function read_options (array $field_definition)
    {
        if (strpos(trim(strtolower($field_definition["type"])), "select use:") === 0) {
            ...
            elseif (count($lookup) == 3) {
                include_once "../classes/tfyh_list.php";
                $list = new Tfyh_list("../config/lists/" . $lookup[1], $lookup[2], "", $this->socket, $this->toolbox);
    ...
    public function get_html (bool $is_file_upload = false, String $get_parameter = "")
    {
        ...
        if ($is_file_upload)
            $form = '		<form enctype="multipart/form-data" action="?fseq=' . $this->fs_id . $this->index . $get_parameter .
                     '" method="post">' . "\n";
        else
            $form = '		<form action="?fseq=' . $this->fs_id . $this->index . $get_parameter . '" method="post">' . "\n";
        // ----------------------------------------------------------
        ...
        foreach ($this->form_definition as $f) {
            ...
            $inline_label = (strcasecmp("radio", $f["type"]) === 0) || (strcasecmp("checkbox", $f["type"]) === 0) ||
            ...
            $preset = (isset($_SESSION["forms"][$this->fs_id][$f["name"]])) ? htmlspecialchars(
            ...
            if (strpos($f["type"], "select") !== false) {
                ...
                if (is_array($options_array))
                    foreach ($options_array as $option) {
                        ...
                        if (strcasecmp($nvp[0], $preset) !== 0)
                            ...
                            $form .= '<option selected value="' . trim($nvp[0]) . '">' . trim($nvp[1]) . "</option>\n";
                    }
                $form .= "</select>\n";
                ...
                foreach ($options_array as $option) {
                    ...
                    $form .= '<input type="radio" name="' . $f["name"] . '" value="' . $nvp[0] . $validity_border_style;
                    if (strcasecmp($nvp[0], $preset) === 0)
                        $form .= '" checked>' . $nvp[1] . '<br>' . "\n";
                    else
                        $form .= '">' . $nvp[1] . '<br>' . "\n";
                ...
                $form .= '<textarea name="' . $f["name"] . '" cols="' . $f["maxlength"] . '" rows="' . $f["size"] . '" ' .
                         $class_str . '>' . $preset . '</textarea><br>' . "\n";
    ...
    public function preset_values (array $values, bool $keep_hidden_defaults = false)
    {
        foreach ($this->form_definition as $f) {
            if (isset($values[$f["name"]]) && (! $keep_hidden_defaults ||
                ...
                $_SESSION["forms"][$this->fs_id][$f["name"]] = $values[$f["name"]];
        }
    }

    /**
     * preset a single value of the form object. If the key is not a field name of the form, this
     * will have no effect. Value must be a UTF-8 encoded String.
     *
     * @param $key String
    ...
    public function check_validity (int $password_rule = 1)
    {
        ...
        foreach ($_SESSION["forms"][$this->fs_id] as $key => $value) {
            $definition = $this->form_definition[$key];
                ...
                if (strlen($value) < 1) {
                    ...
                    if (isset($definition["type"])) {
                        ...
                        if (strcmp($type, "email") === 0) {
                            ...
                            if (strlen($errors) > 0) {
                                $form_errors .= 'Das Passwort ist nicht ausreichend sicher in "' . $definition["label"] .
                                         '" ' . $errors . '<br>';
                                $this->validities[$key] = false;
                            }
                        }
