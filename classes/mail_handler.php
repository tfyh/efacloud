            ...
            $attachment1_location = "", $attachment2_location = "")
    {
        ...
        if (strlen($mailto) > 0) {
            ...
            foreach ($mhelements as $mhelement) {
                if ($i == 0) {
                    $mailto_encoded = Mail_handler::mhLineEncode("To:" . $mhelement);
                    ...
                    $mailheaders_encoded .= Mail_handler::mhLineEncode("To:" . $mhelement);
                }
                $i ++;
            }
        }
        // From may contain a real name, but is a single address
        $mailheaders_encoded .= Mail_handler::mhLineEncode("From:" . $mailfrom);
        // Reply-To shall only be a real mail address, no name to be given
        if (strlen($mailreplyto) > 0) {
            $mailheaders_encoded .= Mail_handler::mhLineEncode("Reply-To:" . $mailreplyto);
        }
        // To; Cc:, Bcc: Can have names
        if (strlen($mailcc) > 0) {
            $mhelements = explode(",", $mailcc);
            foreach ($mhelements as $mhelement) {
                $mailheaders_encoded .= Mail_handler::mhLineEncode("Cc:" . $mhelement);
            }
        }
        if (strlen($mailbcc) > 0) {
            $mhelements = explode(",", $mailbcc);
            foreach ($mhelements as $mhelement) {
                $mailheaders_encoded .= Mail_handler::mhLineEncode("Bcc:" . $mhelement);
            }
        }
        ...
        // find plain text
        $plain = "Für diese Mail gibt es leider keine Klartextalternative.";
